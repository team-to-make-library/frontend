...
export class Controller {
  ...
  static getTopHeight(topEl: HTMLElement | undefined | null) {
    if (!topEl) return 0;
    return Math.floor(topEl.getBoundingClientRect().top);
  }

  static caluculateHeight(childrens: HTMLElement[]) {
    return childrens.reduce<number[]>((acc, curr, i, _arr) => {
      if (i === 0) return [0];
      const prev = acc[i - 1];
  ...
  init({ childrens }: { childrens: HTMLElement[] }) {
    ...
    const startHeight = Controller.getTopHeight(childrens[0]);
